...
| raw surface image / C-scan image | I^S / I^X | 原始声学阵列（本稿未使用该输入） |
| surface/internal descriptors | S / X, each 64×512 | 原始像素 |
| logical visible internal buffer | X_obs = M broadcast-multiplied by X | 未测内部内容 |
| acquired cell set | Omega_t, 8×8共64格，编号0—63 | 声学采样点 |
...
| matched empirical quality | first cohort-level pooled-MAE passage on stated grid | 部署STOP、单件oracle停止 |
...
固定策略执行总预算B=0.25；较小cap是该轨迹的前缀查询。训练反馈P_-k按TRAIN组排除当前折拟合；评价只用P_all。预测器Huber监督batch32，Actor轨迹代价batch16，不混用。
