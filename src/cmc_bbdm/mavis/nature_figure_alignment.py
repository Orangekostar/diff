...
def _inferred_groups(
    panels: Sequence[dict[str, Any]], orientation: str
) -> list[dict[str, Any]]:
    ...
    for index, (_key, panel_ids) in enumerate(grouped.items(), 1):
        if len(panel_ids) >= 2:
            output.append(
                {"id": f"inferred-{orientation}-{index}", "panels": panel_ids}
            )
    return output
...
def _validated_layout(
    ...
    for index, raw_panel in enumerate(raw_panels, 1):
        ...
        if (
            bbox[0] < -0.01
            or bbox[1] < -0.01
            or bbox[2] > width_pt + 0.01
            or bbox[3] > height_pt + 0.01
        ):
            errors.append(
                f"panel {panel_id} bbox_pt extends beyond the declared figure"
            )
        ...
        if anchor is not None:
            try:
                ...
                if len(anchor_values) != 2 or not all(
                    math.isfinite(value) for value in anchor_values
                ):
                    raise ValueError
    ...
    for group in [*row_groups, *column_groups]:
        ...
        if missing:
            errors.append(
                f"group {group['id']} references unknown panels: {', '.join(missing)}"
            )

    exemptions: list[dict[str, Any]] = []
    raw_exemptions = manifest.get("exemptions", [])
    if not isinstance(raw_exemptions, Sequence) or isinstance(
        raw_exemptions, (str, bytes)
    ):
    ...
    for index, exemption in enumerate(raw_exemptions, 1):
        ...
        if isinstance(checks, str):
            checks = [checks]
        if not exemption_panels or any(
            str(panel) not in known_ids for panel in exemption_panels
        ):
        ...
        exemptions.append(
            {
                "panels": [str(panel) for panel in exemption_panels],
                "checks": check_names,
                "reason": reason,
            }
        )
...
def _is_exempt(layout: Mapping[str, Any], panel_id: str, check: str) -> bool:
    for exemption in layout.get("exemptions", []):
        if panel_id in exemption["panels"] and (
            check in exemption["checks"] or "all" in exemption["checks"]
        ):
            return True
    return False
...
def _finding(
    ...
    return {
        ...
        "metric_spreads_pt": {
            metric: round(_metric_spread(values), 6)
    ...
    *,
    ...
    for orientation, group_key, check_name in (
        ...
        for group in layout[group_key]:
            ...
            comparisons += 1
            boxes = {
                panel_id: _bbox(panel_map[panel_id]["bbox_pt"])
            ...
            if (
                max(_metric_spread(values) for values in metrics.values())
                > tolerance_pt
            ):
                findings.append(
                    _finding(kind, group, panel_ids, tolerance_pt, metrics, message)
                )
            ...
            if orientation == "row" and len(group_panel_ids) >= 3:
                ...
                for panel_id in width_candidates:
                    ...
                    try:
                        column_span = float(panel.get("col_stop")) - float(
                            panel.get("col_start")
                        )
            ...
            present_anchors = {
                panel: anchor for panel, anchor in anchors.items() if anchor is not None
            }
            missing_labels = [
                panel for panel, anchor in anchors.items() if anchor is None
            ]
            ...
            if len(present_anchors) >= 2:
                ...
                label_ids = [
                    ...
                    if panel in present_anchors
                    and not _is_exempt(layout, panel, label_check)
                ]
                if len(label_ids) >= 2:
                    ...
                    metrics = {
                        metric_name: {
                            panel: float(present_anchors[panel][coordinate])
                            for panel in label_ids
                        }
                    }
            ...
            gutter_check = (
                "horizontal-gutter" if orientation == "row" else "vertical-gutter"
            )
            gutter_ids = [
                panel
                for panel in panel_ids
                if not _is_exempt(layout, panel, gutter_check)
            ]
            if len(gutter_ids) >= 2:
                if orientation == "row":
                    ...
                    ordered = sorted(
                        gutter_ids, key=lambda panel: boxes[panel][3], reverse=True
                    )
                    gaps = {
    ...
    for group in layout["boundary_groups"]:
        ...
        missing_labels = [
            panel_id for panel_id, anchor in anchors.items() if anchor is None
        ]
        if require_panel_labels and missing_labels:
            findings.append(
                {
    ...
    return {
        ...
        **(
            {
                "errors": [
                    "No comparable row, column or shared-boundary groups were declared or inferred"
                ]
            }
            if not auditable
            else {}
...
def write_json_report(report: Mapping[str, Any], path: str | Path) -> None:
    _atomic_write_text(
        Path(path), json.dumps(report, indent=2, ensure_ascii=False) + "\n"
    )
...
def _matplotlib_panel_label_anchor(ax: Any) -> tuple[str, list[float]] | None:
    for artist in ax.texts:
        ...
        if not re.fullmatch(r"[a-z]", label) or not _font_is_bold(
            artist.get_fontweight()
        ):
            continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--overlay-svg", help="write a QA-only SVG of measured panel rectangles"
    )
    parser.add_argument("--tolerance-pt", type=float, default=DEFAULT_TOLERANCE_PT)
    parser.add_argument(
        "--gutter-tolerance-pt", type=float, default=DEFAULT_GUTTER_TOLERANCE_PT
    )
    parser.add_argument("--require-panel-labels", action="store_true")
    parser.add_argument(
        "--strict", action="store_true", help="make WARN findings blocking"
    )
    parser.add_argument(
        "--self-test", action="store_true", help="run dependency-free core tests"
    )
    return parser
